...
def get_options():
    ...
    parser.add_argument('-dt', '--timestep', type=float, default=1e-5, help='timestep to calculate simulation')
    parser.add_argument('-d', '--duration', type=float, default=1.0, help='length of time to run simulation')
    parser.add_argument('-ef', '--freq', type=float, default=220, help='freqency of signal to put through network')
    ...
    parser.add_argument('-el', '--low', type=float, default=25.0)
    parser.add_argument('-eh', '--high', type=float, default=100.0)
    parser.add_argument('-lim',type=float,default=[24,112], nargs=2)
...
if __name__ == "__main__":
    ...
    for i in range(1,len(tfs)+1):
        print(f"{tfs[i-1]['order']}\t{tfs[i-1]['Grc']}")
    ...
    plt.figure()
    for v in voltages:
        plt.plot(t, v['y'], label=f"Stage-{v['order']}", marker='x')
    ...
    plt.plot(t, modelOutput, label='EWMA-model', marker='o')

    print(f'RESULTANT EWMA CASCADE:{coeff}')
